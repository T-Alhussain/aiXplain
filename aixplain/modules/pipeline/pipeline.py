...
class UtilitiesInputs(Inputs):
    inputs: InputParam = None

    def __init__(self, node=None):
        super().__init__(node=node)
        self.inputs = self.create_param(code="inputs", data_type=DataType.TEXT, is_required=True)


class UtilitiesOutputs(Outputs):
    outputs: OutputParam = None

    def __init__(self, node=None):
        super().__init__(node=node)
        self.outputs = self.create_param(code="outputs", data_type=DataType.TEXT)


class Utilities(AssetNode[UtilitiesInputs, UtilitiesOutputs]):
    """
    ...
    OutputType: text
    """
    function: str = "utilities"
    input_type: str = DataType.TEXT
    output_type: str = DataType.TEXT

    inputs_class: Type[TI] = UtilitiesInputs
    outputs_class: Type[TO] = UtilitiesOutputs
...
performance of tasks, calculations, or operations as defined by the script.
    ...
    def utilities(self, asset_id: Union[str, asset.Asset], *args, **kwargs) -> Utilities:
        """
        
        """
        return Utilities(*args, asset_id=asset_id, pipeline=self, **kwargs)
